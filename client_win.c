...
static SDL_Surface *startsur;//スタート画面を表すバッファ
static SDL_Surface *endsur;//end画面を表すバッファ
static SDL_Surface *scbuf;//得点を表すバッファ
static SDL_Surface *stbar;//スタミナを表すバッファ
static SDL_Surface *gCharaImage;//キャラを表すバッファ
...
static TTF_Font* font;	//TrueTypeフォントデータへのポインタkochi-gothic-subst.ttf,48
static TTF_Font* font2;//kochi-gothic-subst.ttf,24
static TTF_Font* font3;//kochi-gothic-subst.ttf,12
static TTF_Font* font_times;//ゲーム回数表示用のフォントkochi-gothic-subst.ttf,20
...
int InitWindows(void)
{
    ...
    sprintf(title,"Kabaddi[%d]",clientID);//ウインドウのタイトルをセット
    SDL_WM_SetCaption(title,NULL);    

    SDL_FillRect(bufmain,NULL,0xffffffff);//背景を白にする
    
    gMessage_title = TTF_RenderUTF8_Blended(font, "白熱カバッディ", colB);
    SDL_Rect src_rect = { 0, 0, gMessage_title->w,gMessage_title->h };
    gMessage_req = TTF_RenderUTF8_Blended(font2, "スペースキーを押すんだ！", colB);
    ...
    gMessage_chotomate = TTF_RenderUTF8_Blended(font, "ちょっと待ってくれい！", colB);
...
int TopWindow(void)
{
    ...
    if(serectflag==1)
        SDL_BlitSurface(serect, NULL, gMainWindow, &serect_gamestart_rect);//ゲームスタート
    else if(serectflag==2)
        SDL_BlitSurface(serect, NULL, gMainWindow, &serect_end_rect);//全プレイヤーのウィンドウを閉じる
    
    gMessage_explain_2 = TTF_RenderUTF8_Blended(font3, "Select -> Please Pless Button A",colB);//操作説明を画面右下に表示
...
int EndWindow(void)
{
    ...
    int i,j;//得点による並び替えのため
    int bef;//得点による並び替えのため
    int t[]={0,1,2,3,4,5,6,7};//得点による並び替えのため
    int tmp;//得点による並び替えのため
    char rank[64];
    char name[64];
    char score[64];
    ...
    for(i=0;i<cnum;i++){
        ...
        gMessage_rank_on[j] = TTF_RenderUTF8_Blended(font2, rank,colB);//各プレイヤーの順位を表示
        ...
        sprintf(name,"%s",gClients[j].name);//各プレイヤーの名前を表示
        ...
        sprintf(score,"%d",gClients[j].score);//各プレイヤーの得点を表示
    ...
    gMessage_explain = TTF_RenderUTF8_Blended(font3, "Next -> Please Pless Button 2",colB);//操作説明を画面右下に表示
...
{   
        ...
        assert(0<num && cnum<=MAX_CLIENTS);//引き数チェック

        Af = 0;
        dflag = 0;//攻守反転フラグの初期化
        tflag = 0;
        game.restTime = 20000;//残り20000ミリ（20）秒
        lineColor(buffer, 800, 0, 800, 600,0x000000ff);//始点x座標，始点y座標，終点x座標，終点y座標，色
	...
	SDL_FillRect(buffer,NULL,0xffffffff);//背景を白にする
        ...
        for(i=0;i<cnum;i++){
            ...
            sprintf(Pname[i],"     ▼%s",gClients[i].name);//キャラの頭上に名前を表示
        ...
        sprintf(status,"score:%dpt",gClients[clientID].score);//得点の表示
    
        mes = TTF_RenderUTF8_Blended(font2, status, colB);        

        SDL_FillRect(scbuf,NULL,0xffffffff);//背景を白にする 
...
機能	: ウィンドウイベント処理
引数	: int	clientID		: クライアント番号
	  int   now                     : ウィンドウイベント処理に入ってときの残り時間 
出力	: なし
***********************************************************************************/
...
/*************タックル（守備側のみ）**************************************/
                ...
                if(gClients[clientID].tackle == 1){//tackle = 0通常 1反転 2タックルに成功
                    ...
                    if(wiimote.keys.two){
                        ...
                        case up_right_dir:
                            ...
                            gClients[clientID].Bflag ? (chara_rect[clientID].y =1296) :(chara_rect[clientID].y =1008);//通常キャラ画像をタックル画像に変更するためサイズ変更
                        ...
                        case down_left_dir:
                            ...
                            gClients[clientID].poi.x = gClients[clientID].poi.x-5;
                            chara_rect[clientID].x=0;
                            gClients[clientID].Bflag ? (chara_rect[clientID].y = 1152) :(chara_rect[clientID].y =864);//通常キャラ画像をタックル画像に変更するためサイズ変更
                            chara_rect[clientID].w=192;
                            break;
                        case left_dir:
                            gClients[clientID].poi.x = gClients[clientID].poi.x-5;
            ...
            else if(gClients[clientID].tackle >= 1){
                tflag++;
                if((tflag%50) == 0){
            ...
            if(wiimote.keys.up || wiimote.keys.down || wiimote.keys.left || wiimote.keys.right && tflag==0)
            {    
                if(gClients[clientID].ADsta == 1){//攻撃側
                    if(wiimote.keys.one){
                        game.restTime = game.restTime - 20;//スタミナゲージを減らす
                        if(gClients[clientID].tackle != 0){//dflag = 0通常 1反転 2タックル成功された
            ...
            if(wiimote.keys.left){
                if(continueflag==0){//continueflagは連続入力の防止
                    buttonflag=1;//ボタンを押されたらTopWindow実行
                    continueflag=1;
                    serectflag++;
                    if(serectflag==3)
                        serectflag=1;
        ...
        else if(game.flag == 3){//各ピリオド終了.1戦終了ごとに表示する
            if(wiimote.keys.plus){//プラスキーで自分以外の人の結果を表示+
                if(continueflag==0){
                    continueflag=1;
                    resultflag++;
            ...
            if(wiimote.keys.minus){//マイナスキーで自分以外の人の結果を表示-
                if(continueflag==0){
                    continueflag=2;
                    resultflag--;
                    WinDisplay(resultflag);
            ...
            if(wiimote.keys.a){//Aボタンを押した人に参加プレイヤー全員がAボタンを押すまで待機中の画面を表示
                char comment[64];
            ...
            if(wiimote.keys.two)//2ボタンでリスタート
            {
...
void WinDisplay(int ID)//引数clientID,WindowEventからの場合はresultflag
{
    ...
    SDL_Surface *gMessage_explain_p;//plus のp
    SDL_Surface *gMessage_explain_m;//minusのm
    SDL_Surface *gMessage_score;
    ...
    gMessage_explain_a = TTF_RenderUTF8_Blended(font3, "Next -> Please Pless Button A",colB);
